...
namespace Step40
{
  ...
  void LaplaceProblem<dim>::run(unsigned int n_cycles_global, unsigned int n_cycles_adaptive)
  {
	...
	double imbalance = MGTools::workload_imbalance(triangulation);
	
	
        pcout << "n_global_active_cells: "  << triangulation.n_global_active_cells()
        ...
        const double max_vmpeak = dealii::Utilities::MPI::max(stats.VmPeak/1024.0, mpi_communicator);
        const double sum_vmpeak = dealii::Utilities::MPI::sum(stats.VmPeak/1024.0, mpi_communicator);

        pcout << "VMPeak max: " << max_vmpeak <<
              " sum: " << sum_vmpeak << std::endl;
